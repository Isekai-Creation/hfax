...
* Wrapping the model inside a `hfax.nn.AnchoredPolicy` (which runs both the
  model and the reference frozen model)
* Using the `hfax.ckpts.AnchoredPolicyLoader` to restore the weights, so the
  weights are correctly mapped to inside `hfax.nn.AnchoredPolicy`.
...
def get_config():
  ...
  return kd.train.Trainer(
      ...
      # Model definition
      model=hfax.nn.AnchoredPolicy(
          policy=hfax.nn.Gemma3_4B(tokens="batch.tokens", text_only=True),
      ),
      # Load the weights from the pretrained checkpoint
      init_transform=hfax.ckpts.AnchoredPolicyLoader(
          policy=hfax.ckpts.LoadCheckpoint(
