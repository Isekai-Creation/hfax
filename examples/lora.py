...
* `model`: Use `hfax.nn.LoRA()` wrapper to add `LoRA` adapters to the
  model.
* `init_transform`: Use `hfax.ckpts.SkipLoRA()` wrapper to only restore the
  non-LoRA weights.
...
def get_config():
  ...
  return kd.train.Trainer(
      ...
      # weights.
      init_transform=hfax.ckpts.SkipLoRA(
          wrapped=hfax.ckpts.LoadCheckpoint(
      ...
      evals={
          ...
          # test set.
          "sampling": hfax.evals.SamplerEvaluator(
              run=kd.evals.EveryNSteps(1000),
