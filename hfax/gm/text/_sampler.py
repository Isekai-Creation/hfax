...
from hfax.data import _functional
from hfax.nn import _transformer
from hfax.text import _prefill
from hfax.text import _sampler_loop
from hfax.text import _sampling
from hfax.text import _tokenizer
from hfax.typing import _common
...
class Sampler:
  ...
  This is a low-level API. For most use cases, prefer `hfax.text.ChatSampler`
  instead.
  ...
  def sample(
    ...
    Args:
      ...
      last_state: When `return_state=True`, the state can be propagated across
        ...
        `hfax.text.ChatSampler` for a simpler API which handles the state for you.
