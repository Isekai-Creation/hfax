...
def _get_output(model: hfax.nn.Transformer, **kwargs) -> tuple[hfax.nn.Output, Any]:
...
def test_transformer(model_cls: type[hfax.nn.Transformer]):
...
def test_images():
  ...
  model = hfax.nn.Gemma3_4B()  # pylint: disable=missing-kwoa  # pytype: disable=missing-parameter
...
def test_text_only():
  ...
  model = hfax.nn.Gemma3_4B(text_only=True)
...
def test_last_only():
  model = hfax.nn.Gemma3_4B(return_last_only=True)
  tokens = jnp.ones((BATCH_SIZE, SEQ_LEN), dtype=jnp.int32)
  out, params = _get_output(model, tokens=tokens)
