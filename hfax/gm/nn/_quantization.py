...
from hfax.peft import _quantization
import jax
...
class QuantizationAwareWrapper(nn.Module):
  ...
  replaced by their quantization aware training versions. See `hfax.peft`
  ...
  def __kontext_keys__(self) -> dict[str, str]:
    ...
    # hfax.nn.QuantizationAwareWrapper(
    #   model=MyModel(
...
class IntWrapper(nn.Module):
  ...
  The model wrapped will have all it's `nn.Dense`, `nn.Einsum`,... layers
  replaced by their int4 versions. See `hfax.peft` documentation for more
  details.
  ...
  def __kontext_keys__(self) -> dict[str, str]:
    ...
    # Forward the keys from the wrapped model.
    # This allow to define the config as:
    # hfax.nn.QuantizationAwaretrainingWrapper(
    #   model=MyModel(
