...
def test_pad():
  np.testing.assert_array_equal(
      hfax.data.pad([1, 2, 3], max_length=6),
      [1, 2, 3, 0, 0, 0],
  )
  np.testing.assert_array_equal(
      hfax.data.pad([1, 2, 3, 4, 5, 6], max_length=6),
      [1, 2, 3, 4, 5, 6],
  )
  with pytest.raises(ValueError, match="Cannot pad sequence"):
    hfax.data.pad([1, 2, 3, 4, 5, 6, 7], max_length=6)

  np.testing.assert_array_equal(
      hfax.data.pad([1, 2, 3, 4, 5, 6, 7], max_length=6, truncate=True),
      [1, 2, 3, 4, 5, 6],
  )

  arr = np.arange(6).reshape((2, 3)) + 1
  np.testing.assert_array_equal(
      hfax.data.pad(arr, max_length=6),
      [
          [1, 2, 3, 0, 0, 0],
          [4, 5, 6, 0, 0, 0],
      ],
...
def test_seq2seq():
  ...
  out = hfax.data.make_seq2seq_fields(
